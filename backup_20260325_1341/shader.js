...
uniform float blurAmount;  // 0=sharp, 1=very blurry (max 8-pixel radius)
...
// Gaussian-like sampling with variable radius
vec3 blurredTexture(sampler2D tex, vec2 uv, float radius) {
    if (radius < 0.5) return texture(tex, uv).rgb;
    vec3 sum = vec3(0.0);
    float total = 0.0;
    float maxOffset = ceil(radius);
    for (float dx = -3.0; dx <= 3.0; dx += 1.0) {
        for (float dy = -3.0; dy <= 3.0; dy += 1.0) {
            float dist = sqrt(dx*dx + dy*dy);
            float weight = exp(-dist*dist / (2.0 * radius*radius/9.0));
            if (weight > 0.01) {
                sum += texture(tex, uv + vec2(dx, dy) / iResolution.xy).rgb * weight;
                total += weight;
            }
        }
    }
...
void main() {
    ...
    float radius = blurAmount * 15.0;
    vec2 texUV = vec2(UV.x, 1.0 - UV.y);
    vec2 n_flipped = vec2(n.x, -n.y);
    vec3 col = blurredTexture(iChannel0, texUV + n_flipped, radius);
